# Based on https://github.com/MarcoParola/pytorch_sidu/sidu.py
...
def sidu(model: torch.nn.Module, layer_name: str, image: torch.Tensor, device: torch.device = torch.device("cpu")) -> torch.Tensor:
    ...
    with torch.no_grad():
        orig_feature_map = model(input)[layer_name]
        orig_predictions = model(input)
        input_size = input.shape[2:]
        for i in range(input.shape[0]):
            masks = generate_masks(input_size, orig_feature_map[i], 16)
